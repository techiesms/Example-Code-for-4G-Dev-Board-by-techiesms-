/**************************************************************
  ...
  This sketch is for getting GNSS data on Serial
  monitor using 4G Development board by techiesms
...
void loop() {
  ...
  sendData("AT+CGNSSPWR=1", 10000, DEBUG);
  sendData("AT+CGNSSPORTSWITCH=1,1", 1000, DEBUG);

  while (true) {
    //[<time>],[<mode>],[<GPS-SVs>],[<GLONASS-SVs>],[BEIDOU-SVs],[<lat>],[<N/S>],[<log>],[<E/W>],[<date>],[<UTC-time>],[<alt>],[<s peed>],[<course>],[<PDOP>],[HDOP],[VDOP]
    msg = sendData("AT+CGNSSINFO", 1000, DEBUG);
    delay(5000);
  }
}
